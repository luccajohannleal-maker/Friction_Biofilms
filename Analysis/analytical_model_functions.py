...
def plot_data(Lambda,ax):
    ...
    if int(Lambda)==5:
        ...
        ax.errorbar(time5,df5["avg"],yerr=df5["stdev"],c=dfunc.colour_Lambda(5.0),label=r"$simulation$")
    if int(Lambda)==10:
        ...
        ax.errorbar(time10,df10["avg"],yerr=df10["stdev"],c=dfunc.colour_Lambda(10.0),label=r"$simulation$")
...
def plot_average_inner_fraction(t0,tend=6,Lambda=10,ax=None):
    dt = 0.0001
